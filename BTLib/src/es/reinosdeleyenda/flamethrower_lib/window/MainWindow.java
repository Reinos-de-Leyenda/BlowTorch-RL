...
import es.reinosdeleyenda.flamethrower_lib.alias.BetterAliasSelectionDialog;
import es.reinosdeleyenda.flamethrower_lib.service.*;
import es.reinosdeleyenda.flamethrower_lib.service.plugin.settings.BaseOption;
import es.reinosdeleyenda.flamethrower_lib.service.plugin.settings.OptionsDialog;
...
import es.reinosdeleyenda.flamethrower_lib.settings.HyperSettings;
import es.reinosdeleyenda.flamethrower_lib.speedwalk.BetterSpeedWalkConfigurationDialog;
import es.reinosdeleyenda.flamethrower_lib.timer.BetterTimerSelectionDialog;
import es.reinosdeleyenda.flamethrower_lib.trigger.BetterTriggerSelectionDialog;
import es.reinosdeleyenda.flamethrower_lib.ui.SDCardUtils;

import android.support.v7.app.AppCompatActivity;
import android.support.v4.view.MenuItemCompat;

import es.reinosdeleyenda.flamethrower_lib.service.Processor;
...
public class MainWindow extends AppCompatActivity implements MainWindowCallback,ActivityCompat.OnRequestPermissionsResultCallback {
	...
	public void onCreate(Bundle icicle) {
		...
		windowMap = new HashMap<String, Window>(0);

		
		//this.requestWindowFeature(Window.FEATURE_ACTION_MODE_OVERLAY);
		//this
		if(ConfigurationLoader.isTestMode(this)) {
			//Thread.setDefaultUncaughtExceptionHandler(new com.happygoatstudios.flamethrower_lib.crashreport.CrashReporter(this.getApplicationContext()));
		}
...
//					String pWin,String title,String callback,Drawable res
				...
				case MESSAGE_RESETINPUTWINDOW:
					...
					//try {
						...
						Window w = (Window) MainWindow.this.findViewById(MAIN_WINDOW_ID);
						if(w != null) {
							w.jumpToStart();
						}
...
//
		...
		if(!isServiceRunning()) {
			//start the service
			//if("com.happygoatstudios.flamethrower_lib.MainWindow.NORMAL_MODE".equals(this.getIntent().getAction())) {
			//	mode = LAUNCH_MODE.FREE;
			//} else if("com.happygoatstudios.flamethrower_lib.MainWindow.NORMAL_MODE".equals(this.getIntent().getAction())) {
			//	mode = LAUNCH_MODE.TEST;
			//}
			
			/*if(mode == LAUNCH_MODE.FREE) {
				this.startService(new Intent(com.happygoatstudios.flamethrower_lib.service.IStellarService.class.getName() + ".MODE_NORMAL"));
			} else if(mode == LAUNCH_MODE.TEST) {
				this.startService(new Intent(com.happygoatstudios.flamethrower_lib.service.IStellarService.class.getName() + ".MODE_TEST"));
			}*/
			...
			//this.startService(new Intent(com.happygoatstudios.flamethrower_lib.service.IStellarService.class.getName()));
			//servicestarted = true;
		}
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		if(menuStack.size() > 0) {
			Window tmp = (Window)rl.findViewWithTag(menuStack.peek().window);
		...
		if(mWindows != null) {
			for(WindowToken w : mWindows) {
				Window tmp = (Window)rl.findViewWithTag(w.getName());
				tmp.populateMenu(menu);
...
//	}
	...
	public void onBackPressed() {
		...
		if(menuStack.size() > 0) {
			...
			Window w = (Window)rl.findViewWithTag(tmp.window);
	...
	private boolean isServiceRunning() {
		...
		boolean found = false;
		String serviceProcessName = "com.happygoatstudios.flamethrower_lib" + ConfigurationLoader.getConfigurationValue("serviceProcessName", this);
	...
	public void cleanExit() {
		...
		/*if(mode == LAUNCH_MODE.FREE) {
			this.stopService(new Intent(com.happygoatstudios.flamethrower_lib.service.IStellarService.class.getName() + ".MODE_NORMAL"));
		} else if(mode == LAUNCH_MODE.TEST) {
			this.stopService(new Intent(com.happygoatstudios.flamethrower_lib.service.IStellarService.class.getName() + ".MODE_TEST"));
		}*/
	...
	public void onStart() {
		...
		super.onStart();
		/*if("com.happygoatstudios.flamethrower_lib.window.MainWindow.NORMAL_MODE".equals(this.getIntent().getAction())) {
			mode = LAUNCH_MODE.FREE;
		} else if("com.happygoatstudios.flamethrower_lib.window.MainWindow.TEST_MODE".equals(this.getIntent().getAction())) {
			mode = LAUNCH_MODE.TEST;
		}*/
		//if(supportsActionBar()) {
	...
	public void onResume() {
		...
		if(!isBound) {
			...
			if(mode == LAUNCH_MODE.FREE) {
				//this.startService(new Intent(com.happygoatstudios.flamethrower_lib.service.IStellarService.class.getName() + ".MODE_NORMAL"));
				bindService(new Intent(com.happygoatstudios.flamethrower_lib.service.IStellarService.class.getName()+".MODE_NORMAL"), mConnection, 0);
			} else if(mode == LAUNCH_MODE.TEST) {
				//this.startService(new Intent(com.happygoatstudios.flamethrower_lib.service.IStellarService.class.getName() + ".MODE_TEST"));
				this.bindService(new Intent(com.happygoatstudios.flamethrower_lib.service.IStellarService.class.getName()+".MODE_TEST"), mConnection, 0);
			}*/
			SharedPreferences.Editor edit = MainWindow.this.getSharedPreferences("CONNECT_TO", Context.MODE_PRIVATE).edit();
			edit.putString("CONNECT_TO", MainWindow.this.getIntent().getStringExtra("DISPLAY"));
			edit.commit();
	...
	public void initiailizeWindows() {
		...
		if(mWindows == null || mWindows.length == 0) {
			...
			for(Object x : mWindows) {
				...
				Window v = (Window)rl.findViewWithTag(w.getName());
				if(v != null) {
					v.runScriptOnCreate();
				} else {
	...
	private void initWindow(WindowToken w,String dataDir) {
		...
		if(v == null) {
			...
			Window tmp = new Window(dataDir,this,w.getName(),w.getPluginName(),myhandler,w.getSettings(),this);
		...
		for(Object x : mWindows) {
			if(x instanceof WindowToken) {
				...
				if(tmp instanceof Window) {
					try {
						service.unregisterWindowCallback(w.getDisplayHost(), ((Window)tmp).getCallback());
					} catch (RemoteException e) {
						e.printStackTrace();
					}
	...
	public void callWindowScript(String window, String callback) {
		...
		Window lview = (Window)rl.findViewWithTag(window);
		if(lview != null) {
	...
	public boolean checkWindowSupports(String desired, String function) {
		Window window = windowMap.get(desired);
		if(window != null) {
