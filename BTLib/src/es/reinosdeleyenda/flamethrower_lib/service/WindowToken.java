...
public class WindowToken implements Parcelable {
	...
	public WindowToken(final Parcel p) {
		...
		for (int i = 0; i < numLayoutGroups; i++) {
			...
			switch(layoutType) {
			case LAYOUT_SMALL:
				g.setType(LayoutGroup.LAYOUT_TYPE.small);
				break;
			case LAYOUT_NORMAL:
				g.setType(LayoutGroup.LAYOUT_TYPE.normal);
				break;
			case LAYOUT_LARGE:
				g.setType(LayoutGroup.LAYOUT_TYPE.large);
				break;
			case LAYOUT_XLARGE:
				g.setType(LayoutGroup.LAYOUT_TYPE.xlarge);
				break;
			default:
				break;
			}
		...
		mSettings = p.readParcelable(SettingsGroup.class.getClassLoader());
