...
import es.reinosdeleyenda.flamethrower_lib.responder.*;
import es.reinosdeleyenda.flamethrower_lib.responder.ack.*;
import es.reinosdeleyenda.flamethrower_lib.responder.color.ColorAction;
import es.reinosdeleyenda.flamethrower_lib.responder.color.ColorActionEditor;
import es.reinosdeleyenda.flamethrower_lib.responder.gag.GagAction;
import es.reinosdeleyenda.flamethrower_lib.responder.gag.GagActionEditorDialog;
import es.reinosdeleyenda.flamethrower_lib.responder.notification.*;
import es.reinosdeleyenda.flamethrower_lib.responder.replace.ReplaceActionEditorDialog;
import es.reinosdeleyenda.flamethrower_lib.responder.replace.ReplaceResponder;
import es.reinosdeleyenda.flamethrower_lib.responder.script.ScriptResponder;
import es.reinosdeleyenda.flamethrower_lib.responder.script.ScriptResponderEditor;
import es.reinosdeleyenda.flamethrower_lib.responder.toast.*;
...
import es.reinosdeleyenda.flamethrower_lib.window.PluginFilterSelectionDialog;
import es.reinosdeleyenda.flamethrower_lib.responder.TriggerResponder;
import es.reinosdeleyenda.flamethrower_lib.responder.TriggerResponderEditorDoneListener;
import es.reinosdeleyenda.flamethrower_lib.responder.ack.AckResponder;
import es.reinosdeleyenda.flamethrower_lib.responder.ack.AckResponderEditor;
import es.reinosdeleyenda.flamethrower_lib.responder.notification.NotificationResponder;
import es.reinosdeleyenda.flamethrower_lib.responder.notification.NotificationResponderEditor;
import es.reinosdeleyenda.flamethrower_lib.responder.toast.ToastResponder;
import es.reinosdeleyenda.flamethrower_lib.responder.toast.ToastResponderEditor;
...
public class TriggerEditorDialog extends Dialog implements DialogInterface.OnClickListener, TriggerResponderEditorDoneListener {
	...
	public void onCreate(Bundle b) {
		...
		setContentView(es.reinosdeleyenda.flamethrower_lib.R.layout.trigger_editor_dialog);
	...
	private void refreshResponderTable() {
		...
		for(TriggerResponder responder : the_trigger.getResponders()) {
			...
			if(responder.getType() == TriggerResponder.RESPONDER_TYPE.NOTIFICATION) {
				label.setText("Notification: " + ((NotificationResponder)responder).getTitle());
			} else if(responder.getType() == TriggerResponder.RESPONDER_TYPE.TOAST) {
				label.setText("Toast Message: " + ((ToastResponder)responder).getMessage());
			} else if(responder.getType() == TriggerResponder.RESPONDER_TYPE.ACK){
				label.setText("Ack With: " + ((AckResponder)responder).getAckWith());
			} else if(responder.getType() == TriggerResponder.RESPONDER_TYPE.SCRIPT) {
				label.setText("Function: " + ((ScriptResponder)responder).getFunction());
			} else if(responder.getType() == TriggerResponder.RESPONDER_TYPE.REPLACE) {
				label.setText("Replace: " + ((ReplaceResponder)responder).getWith());
			} else if(responder.getType() == TriggerResponder.RESPONDER_TYPE.GAG) {
				label.setText("Gag");
			} else if(responder.getType() == TriggerResponder.RESPONDER_TYPE.COLOR) {
			...
			if(responder.getFireType() == TriggerResponder.FIRE_WHEN.WINDOW_OPEN || responder.getFireType()== TriggerResponder.FIRE_WHEN.WINDOW_BOTH) {
			...
			if(responder.getFireType() == TriggerResponder.FIRE_WHEN.WINDOW_CLOSED || responder.getFireType() == TriggerResponder.FIRE_WHEN.WINDOW_BOTH) {
	...
	private class WindowOpenCheckChangeListener implements CompoundButton.OnCheckedChangeListener {
		...
		public void onCheckedChanged(CompoundButton arg0, boolean checked) {
			if(checked) {
				...
				the_trigger.getResponders().get(position).addFireType(TriggerResponder.FIRE_WHEN.WINDOW_OPEN);
				///Log.e("TEDITOR","TRIGGER TYPE " + the_trigger.getResponders().get(position).getType().getIntVal() + " AT "+ position + " ADDING windowOpen");
			} else {
				the_trigger.getResponders().get(position).removeFireType(TriggerResponder.FIRE_WHEN.WINDOW_OPEN);
	...
	private class WindowClosedCheckChangeListener implements CompoundButton.OnCheckedChangeListener {
		...
		public void onCheckedChanged(CompoundButton arg0, boolean checked) {
			if(checked) {
				//check the closed check state.
				the_trigger.getResponders().get(position).addFireType(TriggerResponder.FIRE_WHEN.WINDOW_CLOSED);
				//Log.e("TEDITOR","TRIGGER TYPE " + the_trigger.getResponders().get(position).getType().getIntVal() + " AT "+ position + " ADDING windowClosed");
			} else {
				the_trigger.getResponders().get(position).removeFireType(TriggerResponder.FIRE_WHEN.WINDOW_CLOSED);
