...
public class NotificationResponder extends TriggerResponder implements Parcelable {
	...
	public boolean doResponse(Context c,TextTree tree,int lineNumber,ListIterator<TextTree.Line> iterator,TextTree.Line line,int start,int end,String matched,Object source,String displayname,String host,int port,int triggernumber,boolean windowIsOpen,Handler dispatcher,HashMap<String,String> captureMap,LuaState L,String name,String encoding) {
		...
		//Intent notificationIntent  = new Intent(c,com.happygoatstudios.flamethrower_lib.window.MainWindow.class);
		...
		//if(mode == LAUNCH_MODE.FREE || mode == LAUNCH_MODE.PAID) {
        	...
        	try {
				w = Class.forName("es.reinosdeleyenda.flamethrower_lib.window.MainWindow",false,cl);
		...
		/*} else {
			notificationIntent = new Intent("com.happygoatstudios.flamethrower_lib.window.MainWindow"+".TEST_MODE");
			String apkName = null;
			try {
				apkName = c.getPackageManager().getApplicationInfo("es.reinosdeleyenda.flamethrowertest", 0).sourceDir;
			} catch (NameNotFoundException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
        	...
        	PathClassLoader cl = new dalvik.system.PathClassLoader(apkName,ClassLoader.getSystemClassLoader());
        	try {
				w = Class.forName("com.happygoatstudios.flamethrower_lib.window.MainWindow",false,cl);
			} catch (ClassNotFoundException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
			...
			try {
				notificationIntent.setClass(c.createPackageContext("es.reinosdeleyenda.flamethrowertest", Context.CONTEXT_INCLUDE_CODE), w);
			} catch (NameNotFoundException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		...
		PendingIntent contentIntent = PendingIntent.getActivity(c, myTriggerId, notificationIntent, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);
